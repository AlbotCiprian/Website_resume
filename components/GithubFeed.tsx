...
export function GithubFeed({ limit = 10 }: { limit?: number }) {
  ...
  const content = useMemo(() => {
    if (loading) {
      return <p className="text-sm text-zinc-400">Loading recent GitHub events...</p>;
    }

    if (error) {
      return <p className="text-sm text-zinc-400">GitHub activity is temporarily unavailable. {error}</p>;
    }

    if (!events.length) {
      return <p className="text-sm text-zinc-400">No recent public GitHub events found.</p>;
    }
        ...
        {events.map((event) => {
          ...
          return (
            <li key={event.id}>
              ...
              >
                <span className="mt-0.5 rounded-full border border-white/15 p-1.5 text-cyan-300">
                  <Icon className="h-4 w-4" />
                </span>
                <span className="flex-1">
                  <span className="block text-sm font-medium text-zinc-100">{event.title}</span>
